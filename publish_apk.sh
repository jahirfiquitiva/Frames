...
if [ "$TRAVIS_PULL_REQUEST" = false ]; then
	if [ "$TRAVIS_TAG" ]; then
		...
		newRelease="$(curl https://api.github.com/repos/${TRAVIS_REPO_SLUG}/releases/tags/${TRAVIS_TAG}"
		releaseId="$(echo "$newRelease" | jq ".id")"

		echo "Publishing APK to tag: $TRAVIS_TAG"
		for apk in $(find *.apk -type f); do
		  apkName="${apk::-4}"
		  printf "Found APK: $apkName\n"
		  printf "Executing: --> https://uploads.github.com/repos/${TRAVIS_REPO_SLUG}/releases/${TRAVIS_TAG}/assets?access_token=${GITHUB_API_KEY}&name=${apkName}.apk"
